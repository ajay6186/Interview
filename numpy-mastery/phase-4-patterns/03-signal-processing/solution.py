# ============================================================================
# Solution 4.3 — Signal Processing
# ============================================================================
...
freq_bins = np.fft.fftfreq(sample_rate, d=1.0/sample_rate)

# Compute the magnitude spectrum (absolute value of fft_result)
...
# Find the dominant positive frequency
pos_mask = freq_bins > 0
dominant_freq = freq_bins[pos_mask][np.argmax(magnitude[pos_mask])]

# ---------------------------------------------------------------------------
# 3. Inverse FFT (reconstruction)
# ---------------------------------------------------------------------------

# Reconstruct the signal from fft_result using np.fft.ifft
...
# Convolve signal2 with kernel using np.convolve with mode='same'
convolved = np.convolve(signal2, kernel, mode='same')
...
# Use np.interp to interpolate at x_new points
x_new = np.array([0.5, 2., 4., 8.])
y_interp = np.interp(x_new, x_sparse, y_sparse)
...
def main():
    ...
    print("Solution 4.3 — All assertions passed!")
