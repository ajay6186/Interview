# ============================================================================
# Solution 3.1 — Strides and Views
# ============================================================================
...
# Store the strides of arr_c
strides_c = arr_c.strides
# For int32 in C-order: strides should be (16, 4) — bytes

# Create the same data in Fortran order and store its strides
arr_f = np.asfortranarray(arr_c)
strides_f = arr_f.strides
...
# Create a view by reshaping base to (3, 4) — should share memory
view_2d = base.reshape(3, 4)

# Create a copy by calling .copy() on the reshaped array
copy_2d = base.reshape(3, 4).copy()
...
# Check if mat is C-contiguous (True/False)
is_c_contiguous = mat.flags['C_CONTIGUOUS']

# Transpose mat — the result is NOT C-contiguous
mat_T = mat.T

# Check if mat_T is C-contiguous
is_T_c_contiguous = mat_T.flags['C_CONTIGUOUS']

# Use np.ascontiguousarray to make mat_T contiguous, store result
...
# Use as_strided to create a (6, 3) view of overlapping windows
windows = as_strided(
    signal,
...
def main():
    ...
    print("Solution 3.1 — All assertions passed!")
