# ============================================================================
# Solution 3.3 — Advanced Indexing
# ============================================================================
...
# Use np.ix_ to select the submatrix formed by rows [0,2,4] and cols [1,3]
row_idx = np.array([0, 2, 4])
col_idx = np.array([1, 3])
ri, ci = np.ix_(row_idx, col_idx)
submatrix = mat[ri, ci]
...
# Create meshgrid from x and y (use default indexing='xy')
...
# Extract the main diagonal as a 1D array
diag_vals = np.diag(square)

# Create a 3x3 diagonal matrix from the array [5., 10., 15.]
diag_mat = np.diag(np.array([5., 10., 15.]))
...
# Get the upper triangular part of m (including diagonal), zero elsewhere
upper = np.triu(m)

# Get the strictly lower triangular part of m (below diagonal only)
lower_strict = np.tril(m, k=-1)
...
# Use np.take to gather elements at indices [2, 0, 4]
taken = np.take(source, [2, 0, 4])

target = np.zeros(6)

# Use np.put to place values [100., 200., 300.] at indices [1, 3, 5]
...
def main():
    ...
    print("Solution 3.3 — All assertions passed!")
