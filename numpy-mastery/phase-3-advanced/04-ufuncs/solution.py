# ============================================================================
# Solution 3.4 — Universal Functions (ufuncs)
# ============================================================================
...
# Use np.add.reduce to compute the sum of arr
add_reduce = np.add.reduce(arr)

# Use np.multiply.reduce to compute the product of arr
mul_reduce = np.multiply.reduce(arr)

# ---------------------------------------------------------------------------
# 2. ufunc.accumulate
# ---------------------------------------------------------------------------

# Use np.add.accumulate to compute the running sum of arr
add_accum = np.add.accumulate(arr)

# Use np.multiply.accumulate to compute the running product of arr
mul_accum = np.multiply.accumulate(arr)
...
# Use np.add.outer to compute all pairwise sums (shape 3x4)
add_outer = np.add.outer(a, b)

# Use np.multiply.outer to compute the outer product (shape 3x4)
...
# Create a custom ufunc using np.frompyfunc(my_clamp, 3, 1)
clamp_ufunc = np.frompyfunc(my_clamp, 3, 1)
...
# Use np.add.reduceat to compute the sum of each segment
segment_sums = np.add.reduceat(data, indices)
...
def main():
    ...
    print("Solution 3.4 — All assertions passed!")
