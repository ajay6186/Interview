...
# Use the mask to extract only passing scores
passing_scores = scores[mask_pass]

# Count the number of passing scores (use mask, not a loop)
pass_count = np.sum(mask_pass)
...
# Select elements at indices [0, 2, 4] using an index array
selected = data[np.array([0, 2, 4])]

# Select in reverse order: indices [5, 4, 3, 2, 1, 0]
reversed_data = data[np.array([5, 4, 3, 2, 1, 0])]
...
# Use np.where to replace negatives with 0, keep positives unchanged
clipped = np.where(values > 0, values, 0)

# Use np.where to label: 1 where values > 0, -1 where < 0, 0 where == 0
labels = np.where(values > 0, 1, np.where(values < 0, -1, 0))
...
# Get the indices of nonzero elements (returns tuple — take [0])
nonzero_idx = np.nonzero(arr)[0]

# Get the index of the maximum element
argmax_idx = np.argmax(arr)

# Get the value of the minimum nonzero element
nonzero_vals = arr[arr != 0]
argmin_val = np.min(nonzero_vals)
...
# Get the indices that would sort unsorted in ascending order
sort_idx = np.argsort(unsorted)

# Use sort_idx to get the sorted array
sorted_arr = unsorted[sort_idx]

# Get indices that sort in descending order
sort_idx_desc = np.argsort(unsorted)[::-1]
...
def main():
    ...
    print("Solution 2.2 — All assertions passed!")
